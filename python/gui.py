...
class SudokuGUI(tk.Tk):
    ...
    def _build_ui(self):
        ...
        for br in range(3):
            for bc in range(3):
                ...
                block.grid(row=br, column=bc, padx=(0 if bc == 0 else 4), pady=(0 if br == 0 else 4))
                for r in range(3):
                    for c in range(3):
                        row_idx = br * 3 + r
                        col_idx = bc * 3 + c
                        e = tk.Entry(block, width=2, font=(None, 18), textvariable=self.cells[row_idx][col_idx], justify='center', bd=1, relief='solid', validate='key', validatecommand=(vcmd, '%P'))
        ...
        tk.Button(btn_frame, text="Paste Image", command=self.paste_image_from_clipboard).pack(side='left', padx=6)
        tk.Button(btn_frame, text="Load Image", command=self.load_image).pack(side='left', padx=6)
        tk.Button(btn_frame, text="Solve", command=self.solve_board).pack(side='left', padx=6)
        tk.Button(btn_frame, text="Reset", command=self.reset_board).pack(side='left', padx=6)
    ...
    def solve_board(self):
        ...
        if not is_board_valid(board):
            messagebox.showerror("Invalid", "Board validation failed (conflicts or too few clues)")
            return
        if solve(board):
    ...
    def load_image(self):
        ...
        start_dir = os.path.join(os.getcwd(), 'upload') if os.path.isdir(os.path.join(os.getcwd(), 'upload')) else None
        path = filedialog.askopenfilename(title='Open Sudoku image', initialdir=start_dir, filetypes=[('Images','*.png;*.jpg;*.jpeg')])
        if not path:
            return
        ...
        try:
            ...
            messagebox.showinfo("Loaded", f"Image loaded: {os.path.basename(path)}\nOCR failed: {e}")
    ...
    def run_ocr_fill(self):
        ...
        board = [[0]*9 for _ in range(9)]
        import numpy as np
        img_bgr = cv2.cvtColor(np.array(self._last_image.convert('RGB')), cv2.COLOR_RGB2BGR)
    ...
    def paste_image_from_clipboard(self):
        ...
        if ImageGrab is None:
            messagebox.showerror('Clipboard error', 'Pillow ImageGrab not available. Install pillow and ensure clipboard access is allowed.')
            return

        try:
            img = ImageGrab.grabclipboard()
        except Exception as e:
            messagebox.showerror('Clipboard error', f'Failed to read clipboard image: {e}')
            return
